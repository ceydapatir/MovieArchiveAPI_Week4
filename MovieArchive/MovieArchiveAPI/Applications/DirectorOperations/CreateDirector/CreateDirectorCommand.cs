...
namespace MovieArchiveAPI.Applications.DirectorOperations.CreateDirector
{
    public class CreateDirectorCommand
    {
        ...
        public CreateDirectorCommand(IMovieArchiveDBContext context, IMapper mapper, DirectorDTO model){
            _context = context;
            _mapper = mapper;
            Model = model;
        }
        ...
        public void Handle(){
            var director = _context.Directors.Where(i => i.Name == Model.Name).Where( i => i.Surname == Model.Surname).SingleOrDefault();
